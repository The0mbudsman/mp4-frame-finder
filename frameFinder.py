...
class frameFinder:
    def __init__(self, clustersize:int=4096, nocleanup:bool=False, unique:bool=False, output_dir_name:str="FrameFinder_output", decoding_buffer:int=99):
        ...
        self.NALU_IDR_HEADER = re.compile(br'\x65|\x45|\x25') # Coded slice of an IDR picture
        self.NALU_SPS_HEADER = re.compile(br'\x67|\x47|\x27') # Sequence Parameter Set options
        self.NALU_PPS_HEADER = re.compile(br'\x68|\x48|\x28') # Picture Parameter Set options
    ...
    def decode_frames_with_header_worker(self, data_zip:list):
        ...
        for frame in complete_iframes:
            ...
            if (mb_in_slice == 0):
                fancyblob += header["data"] + b"\x00\x00\x01" + frame["data"]
            else:
        ...
        process = (ffmpeg
                ...
                .output(f"./{self.OUTPUT_DIR_NAME}/header_{i}/frame%5d.jpg", start_number=(header["saved_count"]+1), 
                    vsync="0", vcodec="mjpeg", loglevel="quiet")
