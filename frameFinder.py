...
def process_image(filepath:str, clustersize:int=4096, nocleanup:bool=False, unique:bool=False, output_dir_name:str="FrameFinder_output"):
    ...
    def register_partial_SPS_PPS_header(i:int, bytes:bytes):
        ...
        PPS_range = 0
        # Luckily we found all of the header so parse the lot
    ...
    def get_iframes(i, cluster:bytes, match:int):
        ...
        if (iframe_size > MIN_IFRAME_SIZE and iframe_size < MAX_IFRAME_SIZE):  # limits determined from tests
            with open (f"./{OUTPUT_DIR_NAME}/header_0/matches.txt", "a") as f:
                f.write(f"match at {i*4096 + match}, {cluster[match-4:match+4].hex()}, size = {iframe_size}\r\n")
    ...
    def decode_iframes(complete_iframes):
        ...
        for i, header in enumerate(complete_h264_headers):
            for frame in complete_iframes:
                ...
                with open(f"./{OUTPUT_DIR_NAME}/blob_{header['saved_count']}", "wb") as f:
                    f.write(blob)
                process = (ffmpeg
                        .input("pipe:")
                        .output(f"./{OUTPUT_DIR_NAME}/header_{i}/frame_{header['saved_count']}.jpg", vframes=1, vcodec="mjpeg", loglevel="quiet")
                        .run_async(pipe_stdin=True)
                        )
                process.communicate(input=blob)
                if os.path.exists(f"./{OUTPUT_DIR_NAME}/header_{i}/frame_{header['saved_count']}.jpg"):
                    header["saved_count"] = header["saved_count"] + 1

    def entropy_of_cluster(labels, base=None):
        value,counts = np.unique(labels, return_counts=True)
        return np.entropy(counts, base=base)

    #########################################################################################
    #########################################################################################
    #########################################################################################
    # Step 0 begins here, initialising some variables
    #########################################################################################
    #########################################################################################
    #########################################################################################
    ...
    if not os.path.exists(f"./{OUTPUT_DIR_NAME}"):
        ...
        while os.path.exists(f"./{original_name}_{dir_num}"):   
            dir_num = dir_num + 1 
        else:
            os.mkdir(f"./{original_name}_{dir_num}")
            OUTPUT_DIR_NAME = f"{original_name}_{dir_num}"
        ...
        for i, cluster in tqdm(enumerate(iter(lambda: f.read(clustersize), b'')), desc="Reading clusters", total=math.ceil(os.path.getsize(filepath)/clustersize)):
            ...
            for box_index in potential_AVCC_boxes:
                ...
                if (box_index > clustersize-100):
                    ...
                    parse_full_SPS_PPS_header(cluster[box_index:box_index+100])
    ...
    for i in range(len(complete_h264_headers)):
        ...
        if not os.path.exists(f"./{OUTPUT_DIR_NAME}"):
            os.mkdir(f"./{OUTPUT_DIR_NAME}")
            print(f"Made Directory ./{OUTPUT_DIR_NAME}")
        if not os.path.exists(f"./{OUTPUT_DIR_NAME}/header_{i}"):
            os.mkdir(f"./{OUTPUT_DIR_NAME}/header_{i}")
            print(f"Made Directory ./{OUTPUT_DIR_NAME}/header_{i}")

    with open (f"./{OUTPUT_DIR_NAME}/header_0/matches.txt", "x") as f:
        f.close()
    #########################################################################################
    ...
    if nocleanup:
        ...
        print(f"Recovered a total of {total_saved} iframes from {len(complete_h264_headers)} header sets")
        print(f"Output is in {OUTPUT_DIR_NAME}")
        print("Finished!")
        exit(1)
    else:
        print("===========================================")
        ...
        for j in tqdm(range(len(complete_h264_headers)), desc="Cleaning up output folders", total=len(complete_h264_headers)):
            ...
            for file in glob(f"./{OUTPUT_DIR_NAME}/header_{j}/*.jpg"):
                if os.path.getsize(file) < av_size/1.5:
                    os.remove(file)
                    cleanup = cleanup + 1
        print(f"Recovered a total of {total_saved} iframes from {len(complete_h264_headers)} header sets but removed {cleanup} images in the cleanup process, totalling {total_saved-cleanup}")
        print(f"Output is in {OUTPUT_DIR_NAME}")
        print("Finished!")
        exit(1)
...
if __name__ == "__main__":
    ...
    process_image(filepath, clustersize, nocleanup, unique, output_dir_name=OUTPUT_DIR_NAME)
